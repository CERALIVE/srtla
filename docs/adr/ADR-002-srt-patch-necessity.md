...
> **Historical record. Superseded in part by decision D10 of the upstream hard-fork plan.**
>
> This ADR is kept verbatim because it is the A/B rationale that the periodic-NAK
> measurement builds on: the pre-registered rule, the reorder-stress instrument
> (`tests/compat/scenarios/reorder-stress.sh`), and the `srt-sink` receiver stand-in
> all originate here. What it decided has since moved on:
>
> - The two libsrt forks it compares no longer exist in that form. CERALIVE/srt now
>   carries the SRTLA behaviours as runtime socket options instead of an unconditional
>   compile-time merge: `SRTO_SRTLAPATCHES = 118` (compat setter),
>   `SRTO_PERIODICNAKGATE = 119` (tri-state: `0` off, `1` filter, `2` suppress) and
>   `SRTO_REORDERFREEZE = 120`. `irl-srt-server` is a byte-identical clone of its
>   irlserver upstream and sets `SRTO_SRTLAPATCHES` only.
> - Which periodic-NAK arm the compat setter applies (`1` filter vs `2` suppress) is
>   decided by **D10**, a measured A/B on this repo's compat harness rather than a
>   paper choice. The frozen rule, arms, netem cells and metric set live in
>   [`tests/compat/scenarios/ab-periodic-nak.yaml`](../../tests/compat/scenarios/ab-periodic-nak.yaml);
>   `tests/compat/lib/ab-verdict.py` is the only thing that turns its `rows.json` into
>   a winner. A tie or a difference inside the noise band resolves to `2` (upstream
>   parity).
> - The "Task 20" / "Step 2" omission path below refers to a plan that has been
>   replaced. Do not act on it; treat sections 3 to 5 as the state of the world when
>   the verdict was recorded.
>
> Nothing below the banner has been edited.
