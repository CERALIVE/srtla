...
import { readTelemetry, SENDER_TELEMETRY_PATH_PREFIX, SENDER_TELEMETRY_STALE_MS, senderTelemetryPath, telemetrySchema, watchTelemetry, } from './index.js';
...
describe('readTelemetry', () => {
    ...
    test('stale snapshot (last_updated_ms = now - 6000) returns null', async () => {
        const p = await writeSnapshot(JSON.stringify({ last_updated_ms: Date.now() - 6000, connections: [] }));
        expect(await readTelemetry(p)).toBeNull();
    });
    test('staleness boundary: exactly 5000ms old is fresh, 5001ms old is stale', async () => {
        ...
        try {
            const atBoundary = await writeSnapshot(JSON.stringify({ last_updated_ms: FIXED - SENDER_TELEMETRY_STALE_MS, connections: [] }));
            expect(await readTelemetry(atBoundary)).not.toBeNull();
            const justOver = await writeSnapshot(JSON.stringify({ last_updated_ms: FIXED - SENDER_TELEMETRY_STALE_MS - 1, connections: [] }));
    ...
    test('round-trip: Task 18 emitted snapshot parses through readTelemetry with correct types', async () => {
        ...
        const golden = JSON.parse(GOLDEN_FULL_JSON);
...
describe('watchTelemetry', () => {
    ...
    test('calls cb with null for an absent file', async () => {
        ...
        const results = [];
        const handle = watchTelemetry(p, (t) => {
            results.push(t);
        }, { intervalMs: 20 });
        await sleep(50);
        handle.stop();
