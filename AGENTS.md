...
- `--smoke` → runs ONE paired cell (candidate `f1-n1-plain` NAK-on vs baseline `f1-n0-plain`) bounded, writing per-rep `result.json`; needs CAP_NET_ADMIN **and** a resolvable `srtla-send-rs` (else SKIP exit 77)
- `--stage screen|deep` → the full screen matrix / deep sweep (this driver runs the candidate cells; the deep-stage adverse-axis + FEC-geometry data **collection** is the **T-A6** seam — but the cross-cell **stats engine** that turns that evidence into a verdict now exists as `--analyze`, below)
