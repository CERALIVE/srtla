...
#                          "build_arg","pin","immutable","repo"}, ..]
#                      "pin" carries the registry's `pin:` (immutable 40-hex) or
#                      its `ref:` (a moving branch we own); "immutable" says
...
TOKEN_ALIAS = {
    ...
    "ceralive-srtla-send-rs": "ceralive-send-rs",
}


def build_ref(entry):
    """The ref an image is built at: an immutable `pin`, or a moving `ref`.

    matrix.yaml allows exactly one of the two per entry (validate-matrix.py
    enforces that). Third-party upstreams carry a 40-hex `pin`; our own
    hard-fork branches, which are still advancing, carry a branch `ref`. Both
    end up in the same build-arg slot, so nothing downstream has to know which
    kind it got."""
