...
// Same rationale as kSrtoReorderFreeze: SRTO_PERIODICNAKGATE is a CERALIVE-only
// option id, absent from stock libsrt headers. Tri-state: 0 off (Haivision
// behaviour), 1 filter, 2 suppress (bit-exact upstream). This is the axis the
// ab-periodic-nak campaign measures; on a libsrt without it the set fails and is
// reported "unsupported", never fatal, and the readback stays -1.
constexpr int kSrtoPeriodicNakGate = 119;
...
struct Result {
  ...
  int periodicnakgate_readback = -1;
};
...
void usage(const char *argv0) {
  std::fprintf(stderr,
               ...
               "[--retransmitalgo 0|1] [--reorderfreeze 0|1] "
               "[--periodicnakgate 0|1|2] [--packetfilter STR]\n",
               argv0);
}
...
bool parse_args(int argc, char **argv, Options &opt) {
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (a == "--periodicnakgate") {
      const char *v = need("--periodicnakgate");
      if (!v) return false;
      opt.periodicnakgate = std::atoi(v);
      if (opt.periodicnakgate < 0 || opt.periodicnakgate > 2) {
        std::fprintf(stderr, "srt-sink: --periodicnakgate must be 0, 1 or 2\n");
        return false;
      }
...
int main(int argc, char **argv) {
  ...
  int periodicnakgate_applied = -1;
  if (opt.periodicnakgate >= 0) {
    int rc = srt_setsockflag(listener, static_cast<SRT_SOCKOPT>(kSrtoPeriodicNakGate),
                             &opt.periodicnakgate, sizeof(opt.periodicnakgate));
    periodicnakgate_applied = (rc == 0) ? 1 : 0;
  }
  ...
  const char *reorderfreeze_str =
      opt.reorderfreeze < 0
          ...
          : (reorderfreeze_applied == 1 ? (opt.reorderfreeze ? "on" : "off")
                                        : "unsupported");
  const std::string periodicnakgate_str =
      opt.periodicnakgate < 0
          ? "default"
          : (periodicnakgate_applied == 1 ? std::to_string(opt.periodicnakgate)
                                          : std::string("unsupported"));
  std::fprintf(stderr,
               "srt-sink: nakreport=%s lossmaxttl=%s retransmitalgo=%s "
               "reorderfreeze=%s periodicnakgate=%s\n",
               ...
               reorderfreeze_str, periodicnakgate_str.c_str());
  ...
  while (!g_stop) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (s == listener) {
        ...
        rblen = static_cast<int>(sizeof(rb));
        if (srt_getsockflag(client, static_cast<SRT_SOCKOPT>(kSrtoPeriodicNakGate),
                            &rb, &rblen) == 0) {
          res.periodicnakgate_readback = rb;
        }
        std::fprintf(stderr,
                     "srt-sink: accepted SRT caller (packetfilter='%s' "
                     "nakreport_readback=%d lossmaxttl_readback=%d "
                     "reorderfreeze_readback=%d periodicnakgate_readback=%d)\n",
                     res.packetfilter.c_str(), res.nakreport_readback,
                     res.lossmaxttl_readback, res.reorderfreeze_readback,
                     res.periodicnakgate_readback);
        continue;
      }
