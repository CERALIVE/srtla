...
#   --validate-only    Validate the registry (matrix.yaml) and every committed A/B
#                      scenario document against the harness schema, then exit.
#                      Runs no stream, needs no docker/ffmpeg/build dir.
...
# --validate-only is answered before anything else so a schema check works on a
# bare checkout (no docker, no ffmpeg, no build dir) — the acceptance gate for
# the committed scenario documents runs in exactly that environment.
for _arg in "$@"; do
  if [[ "$_arg" == "--validate-only" ]]; then
    exec python3 "${SCRIPT_DIR}/validate-matrix.py"
  fi
done
...
reg  ceralive-send-rs   sender    docker  compat/ceralive-send-rs    ceralive-srtla-send-rs ""
