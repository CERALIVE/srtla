...
#   NAKREPORT             0|1 -> appends --nakreport (periodic NAK on/off).
...
#   PERIODICNAKGATE       0|1|2 -> appends --periodicnakgate: 0 off (Haivision), 1 filter,
#                         2 suppress (upstream-exact). The two arms of the ab-periodic-nak
#                         campaign; needs the CERALIVE libsrt that carries the option.
#   REORDER_PCT           phase-ii reorder probability % (default 25).
#   REORDER_CORR          phase-ii reorder correlation % (default 50). Ignored when
#                         REORDER_GAP is set, because netem's gap form takes no correlation.
#   REORDER_GAP           N -> use netem's `reorder <pct>% gap N` form (every Nth packet is a
#                         reorder candidate) instead of the default correlation form. Empty
#                         default keeps the discipline byte-identical to `reorder 25% 50%`.
...
#
# Adverse-config axes. Empty default = the netem disciplines below are
# byte-identical to the no-axis form, so every existing run is unaffected. They
# widen the impairment envelope an A/B arm is stressed against — the
# ab-periodic-nak cells drive STEADY_LOSS_PCT together with the reorder knobs:
...
#
# FEC caller passthrough. Empty default = today's ffmpeg-direct SRT caller,
# byte-identical:
...
#
# Sender selection (an A/B campaign's PRIMARY sender is the Rust fork). Empty
...
#                         if it were production. An A/B campaign sets this so a
#                         missing fork binary SKIPs like a missing compat pair
#                         rather than silently falsifying the campaign. Default unset
#                         = the C sender is used (existing callers unaffected).
#
...
#
# The ab-periodic-nak campaign drives this script with REORDERFREEZE=1
# LOSSMAXTTL=200 on BOTH arms and varies only PERIODICNAKGATE (1 vs 2), so the
# arms differ in exactly one receive-side option. See
# scenarios/ab-periodic-nak.yaml for the frozen arm and cell definitions.
#
...
PERIODICNAKGATE="${PERIODICNAKGATE:-}"
REORDER_PCT="${REORDER_PCT:-25}"
REORDER_CORR="${REORDER_CORR:-50}"
...
[[ -z "$PERIODICNAKGATE" || "$PERIODICNAKGATE" =~ ^[012]$ ]] || die "PERIODICNAKGATE must be 0, 1 or 2"
[[ "$REORDER_PCT" =~ ^[0-9]+$ && "$REORDER_PCT" -le 100 ]] || die "REORDER_PCT must be 0..100"
[[ "$REORDER_CORR" =~ ^[0-9]+$ && "$REORDER_CORR" -le 100 ]] || die "REORDER_CORR must be 0..100"
[[ -z "$REORDER_GAP" || ( "$REORDER_GAP" =~ ^[0-9]+$ && "$REORDER_GAP" -ge 1 ) ]] \
  || die "REORDER_GAP must be a positive integer"

# netem takes the reorder probability either with a correlation or with a `gap N`
# (every Nth packet is a candidate). They are mutually exclusive forms, and the
# default (25% 50%) reproduces the pre-parameterisation discipline exactly.
if [[ -n "$REORDER_GAP" ]]; then
  NETEM_REORDER_ARGS=(reorder "${REORDER_PCT}%" gap "$REORDER_GAP")
else
  NETEM_REORDER_ARGS=(reorder "${REORDER_PCT}%" "${REORDER_CORR}%")
fi
...
# Sender selection. The Rust srtla-send-rs is the production sender and
...
[[ -n "$PERIODICNAKGATE" ]] && SINK_ARGS+=(--periodicnakgate "$PERIODICNAKGATE")
...
# ----------------------------------------------------------------------------- #
log "==> phase ii: netem ${NETEM_REORDER_ARGS[*]} delay ${REORDER_DELAY_MS}ms on link A for ${PHASE_SEC}s${NETEM_SEED:+ (seed ${NETEM_SEED})}"
...
if [[ -n "$NETEM_SEED" ]]; then
  tc qdisc change dev "$HOSTIF" parent 1:1 handle 10: \
     netem delay "${REORDER_DELAY_MS}ms" "${NETEM_REORDER_ARGS[@]}" seed "$NETEM_SEED" "${NETEM_LOSS_ARGS[@]}" 2>/dev/null \
  || tc qdisc change dev "$HOSTIF" parent 1:1 handle 10: \
       netem delay "${REORDER_DELAY_MS}ms" "${NETEM_REORDER_ARGS[@]}" "${NETEM_LOSS_ARGS[@]}" \
...
jq -n \
  ...
  --arg periodicnakgate "${PERIODICNAKGATE:-default}" \
  --arg reorder_discipline "${NETEM_REORDER_ARGS[*]}" \
  ...
  '{
    ...
    config:{bitrate_kbps:$bitrate_kbps, rx_latency_ms:$rx_latency_ms,
            ...
            reorderfreeze:$reorderfreeze, periodicnakgate:$periodicnakgate,
            reorder_discipline:$reorder_discipline, netem_seed:$netem_seed,
