#!/bin/bash
# Compat-test entrypoint for irlserver/srtla_send (Rust).
#
...
echo "[entrypoint] irlserver srtla_send -> ${RECEIVER_HOST}:${RECEIVER_PORT} (local SRT port ${LOCAL_SRT_PORT})"
echo "[entrypoint] uplink source IPs:"
cat "${IPS_FILE}"

# irlserver CLI: srtla_send [OPTIONS] SRT_LISTEN_PORT SRTLA_HOST SRTLA_PORT BIND_IPS_FILE
