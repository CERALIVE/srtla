#!/bin/bash
# Compat-test entrypoint for CERALIVE/srtla-send-rs (Rust sender).
#
...
echo "[entrypoint] ceralive srtla_send -> ${RECEIVER_HOST}:${RECEIVER_PORT} (local SRT port ${LOCAL_SRT_PORT})"
echo "[entrypoint] uplink source IPs:"
cat "${IPS_FILE}"

# CLI: srtla_send [OPTIONS] SRT_LISTEN_PORT SRTLA_HOST SRTLA_PORT BIND_IPS_FILE
